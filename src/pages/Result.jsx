...
function Result() {
  ...
  useEffect(() => {
    const processMeeting = async () => {
      ...
      try {
        console.log("📥 Whisper 백너드에서 결과(Diarization) 가져오는 중...");
        const whisperRes = await axios.get(`${WHISPER_BACKEND_URL}/result`);
        const segments = whisperRes.data;

        let targetTranscript = "";

        if (segments && Array.isArray(segments) && segments.length > 0) {
          // [핵심] 화자 분리된 전체 텍스트 생성
          targetTranscript = segments
            .map(s => `[${s.speaker}] ${s.text}`)
            .join("\n\n");
          console.log("✅ Whisper 결과 수신 성공");
        } else {
          // 결과가 없으면 로컬 스토리지나 컨텍스트에서 시도
          targetTranscript = localStorage.getItem("lastTranscript") || transcript;
        }

        if (!targetTranscript) {
          console.log("🧪 테스트 데이터 주입");
          targetTranscript = "[김프로] 안녕하세요, 진행 상황 점검입니다. [이기획] 거의 다 되었습니다.";
        }

        // Context 업데이트
        // setTranscript(targetTranscript); // useAppContext에서 제공하는 경우

        console.log("🧠 Main Backend에 분석 요청 중...");

        const response = await axios.post(`${API_URL}/analyze-meeting`, {
          summary_text: targetTranscript,
        });
